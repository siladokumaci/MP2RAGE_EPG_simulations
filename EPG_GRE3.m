function [F0,Fn,Zn,F] = EPG_GRE3(theta,phi,TR,T1,T2,varargin)
%   [F0,Fn,Zn,F] = EPG_GRE3(theta,phi,TR,T1,T2,varargin)
%   This part can be the second loop following an EPG_GRE loop
%   If it is used as the first loop, it will be assumed that 1 RF pulse has
%   already been applied before resulting in 3 additional rows of zeroes.
...
for ii=1:length(varargin)
    ...
    % PD - added by Sila Dokumaci on 13.11.2020
    if strcmpi(varargin{ii},'zinit')
        zinit = varargin{ii+1};
    end
    
    % Initial State - added by Sila Dokumaci on 24.02.2020
    if strcmpi(varargin{ii},'finit')
        finit = varargin{ii+1};
    end
end
...
% np = length(theta);
% two lines below were added by Sila Dokumaci on 24.02.2020
previous_np = length(finit)/3;% there were already some pulses applied
%previous_np = previous_np - 1;% just for EPG_GRE code and EPG_GRE3 code comparison purposes
np = length(theta) + previous_np;
...
%%% regrowth
b = zeros([N 1]);
b(3) = zinit*(1-E1);%<--- just applies to Z0 % zinit added by Sila Dokumaci on 13.11.2020
...
np2 = length(theta);% added by Sila Dokumaci on 24.02.2020
F = zeros([N np2]); %%<-- records the state after each RF pulse
% the line above is originally F = zeros([N np2]) but np2 here is equal to
% np in the original code

% Initial State from previous run - added by Sila Dokumaci on 24.02.2020
if exist('finit','var') 
    FF = zeros([N 1]);
    FF(1:length(finit),1)=finit;   
end
...
%if exist('zinit','var')
%    FF(3)=zinit;   % Added by Sila Dokumaci on 24.02.2020 for variable M0
%end
...
if exist('prep','var')
    ...
    % The line above was changed by Sila Dokumaci on 25.02.2020 below
    p_prep = 0;
    FF = reshape(FF,[3 length(FF)/3]);
    FF = RF_rot(prep.flip,p_prep)*FF;
    FF = FF(:);
    % Now apply time evolution during delay period
    % E1p = exp(-prep.t_delay/T1); 
    % FF(3) = E1p * FF(3) + (1-E1p);
    % The lines above were changed by Sila Dokumaci on 25.02.2020 below
    E1_2 = exp(-prep.t_delay/T1);
    E2_2 = exp(-prep.t_delay/T2);
    % E_2 = diag([E2_2 E2_2 E1_2]);
    %%% regrowth
    b2 = zeros([N 1]);
    b2(3) = zinit*(1-E1_2);%<--- just applies to Z0 % M0 multiplication factor was added by Sila Dokumaci on 13.11.2020
    E_2 = spdiags(repmat([E2_2 E2_2 E1_2],[1 kmax+1])',0,N,N);
    for i=1:N
        FF(i) = E_2(i,i)*FF(i)+b2(i);
    end
end
...
% for jj=1:np
count = 1;
for jj=previous_np+1:np
    %%% RF transition matrix
    % A = RF_rot(theta(jj),phi(jj));
    A = RF_rot(theta(count),phi(count));% changed by Sila Dokumaci on 24.02.2020
    ...
    F(kidx,count)=T(kidx,kidx)*FF(kidx);% changed by Sila Dokumaci on 24.02.2020
    if jj==np
        break
    end
    ...
    FF(kidx) = ES(kidx,kidx)*F(kidx,count)+b(kidx);% changed by Sila Dokumaci on 24.02.2020
    
    % Deal with complex conjugate after shift
    FF(1)=conj(FF(1)); %<---- F0 comes from F-1 so conjugate 
    count = count + 1;% added by Sila Dokumaci on 24.02.2020
end
